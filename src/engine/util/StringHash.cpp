...
// dsstofile @ 0x0025299c — per-character normalization for FileStringHash.
// backslash -> forward slash; A-Z -> a-z; all other bytes unchanged.
static int dsstofile(int c) {
    if (c == '\\') return '/';
    if ((unsigned)(c - 'A') < 26u) c += 0x20;
    return c;
}

// ASM-spec v1.6.1 FileStringHash @ 0x00252e98:
//   Jenkins lookup3 hash for file paths. Same seeds as StringHash
//   (a=b=0x9e3779b9, c=0x805, c+=len). Per-char: dsstofile (backslash->slash + A-Z->a-z).
//   Bit-exact with StringHash for lowercase-no-backslash inputs.
uint32_t FileStringHash(const char* str) {
    ...
    while (remaining > 11) {
        for (int i = 0; i < 12; i++) {
            buf[i] = (uint8_t)dsstofile((int)p[i]);
        }
        p += 12;
...
// ASM-spec v1.6.1 MemHash @0x00252c88: Bob Jenkins lookup3 hash over raw bytes;
// byte-identical to StringHash(const char*, int) except does NOT lowercase 'A'..'Z'.
// a=b=0x9e3779b9, c=0x805 (same seed), c += len before tail switch, same final mix.
