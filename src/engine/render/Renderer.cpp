...
// 3D mesh shaders
// Vertex color (attribute 2, a_color) replicates original GL_MODULATE: texture × vertex_color.
// If a mesh has no color data in its vertex stream, the draw code sets a_color = (1,1,1,1)
// as a constant attribute so the texture is passed through unmodified.
// Ref: PassBinding::Apply (0x001a39f8), GL_MODULATE semantics, Tier 1 fix.
...
bool Renderer::init() {
    ...
    // 3D shader
    {
        ...
        // Attribute layout: 0=pos, 1=normal, 2=color (GL_MODULATE), 3=uv
