...
void BombBlast::DrawActiveBlasts() {
         ...
         ++it)
    {
        ...
        for (int i = 0; i < VERTS_PER_BLAST; ++i) {
            ...
            v[i].nz = 1.0f;
            v[i].colour = col;
            v[i].u = 0.5f;   // binary: all UVs at one texel; 0.5 samples centre
            v[i].v = 0.5f;
        }
    ...
    g_BombTexture->Set();
    glEnable(GL_BLEND);
    // Additive for the bright-outward feel. Binary inherits blend state
    // from Texture::Set — we pick a reasonable default.
