...
// Loaded by Fruit::LoadInfo (0x17987c, 509 lines)
...
struct FruitInfo {
    ...
    char m_Singular[64];          // +0x040: "singular" attr (localisation key)
    char m_ModelName[64];         // +0x080: "modelName" attr (fallback: m_Name)
    char m_FactTexture[64];       // +0x0C0: "factTexture" attr (fallback: m_Name)
    char m_Plural[64];            // +0x100: "plural" attr (fallback: sprintf("%ss", m_Name))
    ...
    char m_SingularEnglish[64];   // +0x200: "singularEnglish" attr (fallback: m_Name)

    // Colour fields
    uint8_t m_FruitColour[4];    // +0x240: "colour" attr BGRA (parsed from "R,G,B,A")
    // +0x244..+0x24F: float fields
    float m_CollisionScale;       // +0x244: "collision" attr (default 25.0)
    float m_Scale;                // +0x248: "scale" attr (default 1.0)
    ...
    // Flags
    uint8_t m_bHasSplatSeeds;     // +0x26C: "hasSplatSeeds" attr
    uint8_t pad_26d[3];
    ...
    // +0x278: pluralEnglish (128 bytes)
    char m_PluralEnglish[128];    // +0x278: "pluralEnglish" attr

    // +0x2F8: factColour (BGRA)
    uint8_t m_FactColour[4];     // +0x2F8: "factColour" attr (R,G,B → B,G,R,0xFF)
    uint8_t m_bOnSide;           // +0x2FC: "onSide" attr
    uint8_t pad_2fd[3];
