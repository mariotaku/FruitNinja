...
IMPORTANT: The binary shifts text UPWARD (positive local-Y), not downward. See
"Section: Vertical Alignment Root Cause" below for ARM proof and port fix.

### Vertical Alignment Root Cause (2026-04-27)

<!-- Analysed: 2026-04-27T07:00 -->

#### TL;DR

The port's `startY -= factor` (negative) has the **wrong sign**. The binary
shifts text UPWARD via `MatrixStack::TranslateLocal(0, +factor, 0)`. The
correct port code is `startY += factor`.

#### ARM-confirmed formula (from disassembly of 0x00199920-0x00199964)

```arm
00199920: ldr r1,[sp,#0x194]        ; r1 = alignment flags
00199922: tst r1,#0xc               ; if (flags & 0xC) == 0: skip
00199926: beq 0x00199968            ;
00199928: vsub.f32 s19,s19,s24      ; s19 = cursor_y - maxWidth (s24=1.0f)
0019992c: ldr r2,[sp,#0x190]        ; r2 = maxWH Vec2 pointer
0019992e: vldr.32 s14,[r2,#0x4]     ; s14 = maxWH.y_modified (pre-divided by scale)
00199932: tst r1,#0x4               ; if (flags & 0x4): use 0.5 factor
00199942: vneg.f32 s14,s14          ; s14 = -maxWH.y_modified
00199946: vmov.f32 s15,0x3f800000   ; s15 = 1.0f (default factor)
0019994a: vsub.f32 s14,s14,s19      ; s14 = -maxWH.y_modified - (cursor_y - maxWidth)
0019994e: it ne                     ;   if flags & 0x4 != 0:
00199950: vmov.ne.f32 s15,s13       ;     s15 = 0.5f
00199958: vmul.f32 s1,s14,s15       ; s1 = s14 * factor
0019995c: blx Vec3::Vec3            ; Vec3(0.0, s1, 0.0) = TranslateLocal_Vec
00199960: ...
00199964: blx MatrixStack::TranslateLocal ; applied BEFORE world Translate(pos)
```

For ShopListItem title call (`flags=0xE`, `maxWidth=1.0`, single-line
`cursor_y=0`, `maxWH.y=0.0` -- see below):

```
cursor_y - maxWidth = 0.0 - 1.0 = -1.0
-maxWH.y_modified - (-1.0) = 0.0 + 1.0 = 1.0
TranslateLocal_Y = 1.0 * 0.5 = +0.5   (POSITIVE, upward in local space)
```

`MatrixStack::TranslateLocal` calls `LocalTranslate44` which does
`M[3][col] += ty * M[1][col]`. After `Scale(scale, scale, 1)`, this adds
`ty * scale = 0.5 * scale` to the world Y translation column. Confirmed from
binary `_Matrix44<float>::LocalTranslate44 @ 0x0019a3d4`:
```c
this->data[3][1] += 0*... + ty * M[1][1] + 0*... = ty * scale
```

#### maxWH.y = 0.0 (binary-confirmed)

`CopyGlobalVec2_GameTask` reads from a Vec2 pointer stored at GOT_base +
0x78c0 in the GameTask static block. That same pointer is initialized in
`_GLOBAL__I_Achievements_cpp` (init_array entry at 0x001097a9, ARM confirmed):

```c
// _GLOBAL__I_Achievements_cpp, lines 49-54:
iVar2 = DAT_001099cc;    // = 0x78c0 (offset into GOT area)
pfVar3 = *(float **)(iVar9 + iVar2);
pfVar3[1] = DAT_001099a0;  // DAT_001099a0 = 0x00000000 = 0.0f
*pfVar3   = DAT_001099a0;  // both components set to 0.0f
```

The Vec2 is permanently (0.0f, 0.0f). No code writes any other value to it.

#### World Y offset formula (with maxWH.y=0)

```
Binary world_Y = scale * cy + 0.5 * scale + pos.y
Port world_Y   = scale * (cy - 0.5) + pos.y        (from startY = -0.5)
Difference     = scale   (binary renders HIGHER than port)
```

For HD font / scale=20: diff = **20 world units** -- matches user's reported ~20-px offset exactly.
For SD font / scale=25: diff = **25 world units**.

#### What the binary's vertical alignment semantics actually mean

`flags & 0xC` vertical alignment places the text **above** the anchor by
`factor * scale` world units (half-line for `0x4`; full-line for `0x8`; both
flags `0xC` still use the 0.5 factor because `0xC & 0x4 != 0`):

| flags bits 2-3 | factor | World Y shift from anchor |
|----------------|--------|--------------------------|
| 0x0 (none)     | n/a    | 0 (cursor AT anchor)     |
| 0x4 (middle)   | 0.5    | +0.5 * scale UP          |
| 0x8 (bottom)   | 1.0    | +1.0 * scale UP          |
| 0xC (both)     | 0.5    | +0.5 * scale UP          |

In Mortar's Y-up coordinate system (positive Y = up/right in landscape), this
means the text block floats ABOVE the anchor for flags=0x4. The port mistakenly
pushes the cursor DOWN (-0.5), so text renders a full line-height BELOW where
it should be.

#### Port fix (Font.cpp lines ~233-238)

Change `startY -= ...` to `startY += ...`:
...
// CORRECT (matches binary TranslateLocal +0.5 or +1.0):
if (alignment & 0xC) {
    if (alignment & 0x4) {
        startY += normLineH * 0.5f;   // = +0.5, shifts text UP in Y-up world
    } else {
        startY += normLineH;           // = +1.0 for flags & 0x8 only
    }
}
```

This fix moves the rendered text UP by 1.0 * scale world units relative to
the current (wrong) port output, which exactly cancels the binary's
`TranslateLocal(+0.5) - port's(-0.5) = 1.0` discrepancy.

The port uses a per-glyph DrawQuad path (not the binary's DrawTriStrip batch),
but because `startY` is embedded into `cy` before the glyph loop, the sign
flip applies identically to all glyphs.

No caller change is needed. No ShopListItem Y offset adjustment is needed.
