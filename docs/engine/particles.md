...
| Manager::Draw | ✅ ported (Tier A+C) | Layer filtering via `template->m_UseDepth`; three-pass call from `GameDraw` (background=1 → 3D → mid=0 → HUD → foreground=-1). Two-segment start→mid→end colour/size lerp. Blend mode applied via `glBlendFunc(SRC_ALPHA, template->m_BlendMode)`. **GridLock** position snap around (480,320) HUD origin for `rim_spark`. **RotCycle** sin-wave rotation offset. **CycleX / CycleY** cos-wave size modulation. **Not ported:** friction sin/cos (written but unread in binary Draw — likely dead), sub-step integration (irrelevant at fixed dt=1/60), Angular shape branch. |
| AddParticle | ✅ ported (Tier A+B+C) | Shape-type branching: **Point** (0), **Vertex** (1, `pos -= vel`), **Direction** (2, `rotation += atan2(vel.y, vel.x)`). Spin rate linearly lerps start→end over lifetime. RotCycle / CycleX / CycleY rates and random starting phase initialised from template. **Not ported:** Angular (3) — needs `m_field38` state; friction init (dead in binary Draw). |
