...
- **Platform-specific files: `*SDL.cpp` suffix.** SDL/platform-bound code lives in suffix-named files (mirrors the binary's `*Bada` convention — `InputDeviceBada`, `DisplayManagerBada`, `BadaSound`, etc.). Examples in tree: `mainSDL.cpp`, `GameSDL.cpp`, `SoundManagerSDL.cpp`, `gl_funcsSDL.cpp`, `DisplayManagerSDL.cpp`, `InputTranslatorSDL.{h,cpp}`. Public headers (`*.h`) must NOT include `<SDL.h>` or expose SDL types directly — use `void*` (with a comment naming the real SDL type) or `uint32_t` for `SDL_AudioDeviceID`. Cast at the SDL boundary inside the `*SDL.cpp` companion. The `symbol-diff` skill cross-build skips `*SDL.cpp` by pattern (`find src -name "*.cpp" ! -name "*SDL.cpp"`), which keeps the diff focused on portable code and means there's no SDL stub header to maintain in `tools/asm-verify/cross-headers/`.
