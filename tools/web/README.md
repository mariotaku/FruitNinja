...
- **`regen-shell.py`** — fast shell-only regen of `index.html` from the current `shell.html`, reusing the already-built hashed js/wasm/data on disk (no emcc/Docker, <1s). Only valid when no `.cpp`/`.h` changed since the last real build — see the script's own header for the `{{{ SCRIPT }}}` substitution it replicates. `python3 tools/web/regen-shell.py [build/web]`.
