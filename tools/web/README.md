...
- **`build_pages.py`** — assemble the GitHub Pages output dir (`pages/`) from `build/web/`: the game at the site root (`index.html` + hashed `fruit-ninja-<sha8>.{js,wasm,data}` + `splash-<sha8>.webp` + the three unhashed webp buttons + `.nojekyll`) — no landing page, no galleries. `python tools/web/build_pages.py [--out <dir>]`. Idempotent (removes+recreates `pages/`); errors if `build/web/index.html` is missing.
