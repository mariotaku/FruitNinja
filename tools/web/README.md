...
- **`stage-web-assets.py`** — build-phase asset staging (run by the `fn_web_asset_staging` CMake target before `fruit-ninja` links): mirrors `FruitNinjaBada/Data` into `build/web-staging/Data`, transcoding sfx `.wav.pcm` -> Ogg/Vorbis (+ `sfx/sfx-loops.json` loop map), re-encoding Tex1 textures -> WebP-in-`.tex`, and subsetting `gangofchinese.ttf` to the used glyph set; everything else is copied verbatim. Self-provisions its tools (`apt-get install`s ffmpeg / fonttools when missing inside the container). `python3 stage-web-assets.py <repo_root>`.
